...
struct NonStreamView: View {
  ...
  var body: some View {
    ZStack {
      ...
      VStack {
        ...
        VStack(spacing: 12) {
          ...
          Text("Stream from your Ray-Ban Meta glasses and get real-time AI vision analysis on your phone.")
            .font(.system(size: 15))
    ...
    .sheet(isPresented: $showAISettings) { SettingsView(settings: settings) }
...
struct UpdateRequiredMessage: View {
  ...
  private var message: String {
    if showFirmwareUpdate && showDATAppUpdate {
      return "Your glasses firmware and app need updates before Sightread can start."
    }
    if showFirmwareUpdate {
      return "Your glasses firmware needs an update before Sightread can start."
    }
    return "The app on your glasses needs an update before Sightread can start."
  }
  ...
  var body: some View {
    VStack(spacing: 24) {
      ...
      VStack(spacing: 12) {
        TipItemView(
          resource: .videoIcon,
          text: "First, Sightread needs permission to use your glasses camera."
        )
        TipItemView(
          resource: .tapIcon,
