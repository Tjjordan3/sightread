import { sanitizeSourceUrl } from "../lib/safeUrl";

export interface SourceLinkItem {
  title: string;
  url: string;
  snippet?: string;
}

interface SourceLinksProps {
  sources: SourceLinkItem[];
  variant?: "chat" | "vision";
  showSnippets?: boolean;
}

function hostnameFromUrl(url: string): string {
  try {
    return new URL(url).hostname.replace(/^www\./, "");
  } catch {
    return url;
  }
}

export function SourceLinks({
  sources,
  variant = "chat",
  showSnippets,
}: SourceLinksProps) {
  if (sources.length === 0) return null;

  const revealSnippets = showSnippets ?? variant === "vision";

  return (
    <div className={`source-links source-links--${variant}`}>
      <p className="source-links__heading">Sources</p>
      <ol className="source-links__list">
        {sources.map((source, index) => {
          const safeHref = sanitizeSourceUrl(source.url);
          const title = source.title || source.url;
          const host = hostnameFromUrl(source.url);

          return (
            <li key={`${source.url}-${index}`} className="source-links__item">
              {safeHref ? (
                <a
                  className="source-links__link"
                  href={safeHref}
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <span className="source-links__index">[{index + 1}]</span>
                  <span className="source-links__title">{title}</span>
                  <span className="source-links__host">{host}</span>
                </a>
              ) : (
                <span className="source-links__link source-links__link--text-only">
                  <span className="source-links__index">[{index + 1}]</span>
                  <span className="source-links__title">{title}</span>
                  <span className="source-links__host">{host}</span>
                </span>
              )}
              {revealSnippets && source.snippet && (
                <p className="source-links__snippet">{source.snippet}</p>
              )}
            </li>
          );
        })}
      </ol>
    </div>
  );
}
